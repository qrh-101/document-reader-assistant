...
// 获取报告详情
export const getReportDetail = (reportId: string): Promise<ApiResponse<{ report_id: string; content: string; report_metadata?: any; created_at?: string }>> => {
  return get<{ report_id: string; content: string; report_metadata?: any; created_at?: string }>(`/reports/${reportId}`)
}

// 下载报告
export const downloadReport = (reportId: string): Promise<void> => {
  return download(`/download_report/${reportId}`)
}
